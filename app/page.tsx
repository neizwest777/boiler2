// @ts-nocheck
"use client";

import { useState } from "react";

import Header from "@/components/shared/Header";
import Footer from "@/components/shared/Footer";

import {
  LandingPrimaryImageCtaSection,
  LandingShowcase,
  LandingShowcaseItem,
  LandingProductFeaturesGrid,
  LandingProductFeature,
  LandingFaqCollapsibleSection,
  LandingAboutSection,
  LandingProductSteps,
  LandingTestimonialGrid,
  LandingSaleCtaSection,
  LandingBandSection
} from "@/components/landing";

import { Button } from "@/components/shared/ui/button";
import Link from "next/link";
import { Droplets, Phone, Sparkles, Wrench } from "lucide-react";
import { ContactFormDialog } from "@/components/contact/contact-form-dialog";

export default function Page() {
  const [contactOpen, setContactOpen] = useState(false);

  return (
    <>
      <Header className="mb-4" />

      {/* HERO */}
      <LandingPrimaryImageCtaSection
        title="Boileri hooldus, paigaldus ja remont Tallinnas"
        description="Kutsuge kogenud torumees! Kiire ja usaldusväärne teenus üle Tallinna ja Harjumaa. Pakume professionaalset boileri paigaldust, remonti ja hooldust."
        imageSrc="/static/images/1762809808835-imageSrc_bcf7bf97-5006-48e0-a7ea-0f11b05fc88d.png"
        imageAlt="Boiler service"
        withBackground={false}
        variant="primary"
        minHeight={350}
      >
        {/* Красная статичная кнопка */}
        <div
          className="bg-red-600 text-white text-lg font-semibold px-6 py-3 rounded-xl shadow-md cursor-default select-none"
          style={{ pointerEvents: "none" }}
        >
          Helista: 53684587
        </div>

        {/* Рабочая кнопка */}
        <Button
          size="xl"
          variant="outlinePrimary"
          onClick={() => setContactOpen(true)}
        >
          Tasuta konsultatsioon
        </Button>
      </LandingPrimaryImageCtaSection>

      {/* SHOWCASE */}
      <LandingShowcase
        title="Usaldusväärne partner"
        description="Töötame koos juhtivate tootjate ja partneritega"
        textPosition="center"
        withBackground
        variant="primary"
      >
        <LandingShowcaseItem>
          <Wrench className="w-8 h-8" />
        </LandingShowcaseItem>
        <LandingShowcaseItem>
          <Droplets className="w-8 h-8" />
        </LandingShowcaseItem>
        <LandingShowcaseItem>
          <Sparkles className="w-8 h-8" />
        </LandingShowcaseItem>
        <LandingShowcaseItem>
          <Phone className="w-8 h-8" />
        </LandingShowcaseItem>
      </LandingShowcase>

      {/* SERVICES */}
      <LandingProductFeaturesGrid
        title="Meie teenused"
        description="Pakume laia valikut boileri ja torustiku teenuseid"
        withBackground={false}
        variant="primary"
      >
        <LandingProductFeature
          title="Boileri paigaldus"
          description="Ekspertide poolt teostatud paigaldus kõikidele boileri tüüpidele."
          imageSrc="/static/images/1762810111398-imageSrc_c0d78223-d162-4fbb-ad8a-f5e7ce63ce65.png"
        />

        <LandingProductFeature
          title="Boileri remont"
          description="Kiire ja usaldusväärne remonditeenus kõigile boileritele."
          imageSrc="/static/images/1762810822501-imageSrc_08e474ec-3188-4011-b875-e572f86e4fa8.png"
        />

        <LandingProductFeature
          title="Puhastus ja hooldus"
          description="Regulaarne hooldus pikendab boileri eluiga ja tagab ohutu töö."
          imageSrc="/static/images/1762810954990-imageSrc_a9687eae-bf20-4a03-90a7-1080d885cb7e.png"
        />

        <LandingProductFeature
          title="Hädaabi 24/7"
          description="Ööpäevaringne kiire abi boileri rikete korral."
          imageSrc="/static/images/1762811329870-imageSrc_8f087ec4-3ec0-4ebf-acb6-25fab6a24cf1.png"
        />
      </LandingProductFeaturesGrid>

      {/* FAQ SERVICES */}
      <LandingFaqCollapsibleSection
        title="Teenuste detailid"
        description="Vaadake lähemalt meie pakutavaid teenuseid"
        faqItems={[
          {
            question: "Boileri paigaldus",
            answer:
              "Professionaalne paigaldus kõikidele boileri tüüpidele koos garantiiga."
          },
          {
            question: "Boileri remont",
            answer:
              "Kiire diagnostika, varuosade vahetus ja töögarantii."
          },
          {
            question: "Boileri hooldus",
            answer:
              "Katlakivi eemaldus, anoodi vahetus ja täielik tehniline kontroll."
          },
          {
            question: "Hädaabi",
            answer:
              "24/7 reageerimine lekete ja rikete korral."
          }
        ]}
        withBackground
        variant="primary"
      />

      {/* ABOUT */}
      <LandingAboutSection
        title="Meist"
        description="Oleme litsentseeritud, kogenud ja kohalik torustikuettevõte, mis pakub tipptasemel boileri teenuseid Tallinnas ja Harjumaal."
        imageSrc="https://images.unsplash.com/photo-1560179707-f14e90ef3623?w=500&q=80"
        withBackground={false}
      />

      {/* STEPS */}
      <LandingProductSteps
        title="Kuidas see töötab"
        description="Lihtne ja mugav teenuseprotsess"
        withBackground
        withBackgroundGlow
        variant="primary"
      >
        <LandingProductFeature
          title="1. Võtke meiega ühendust"
          description="Helistage või kirjutage — vastame kiiresti."
          imageSrc="https://images.unsplash.com/photo-1423666639041-f56000c27a9a?w=500&q=80"
        />
        <LandingProductFeature
          title="2. Saate hinnapakkumise"
          description="Anname teile läbipaistva ja selge hinnapakkumise."
          imageSrc="/static/images/1762813449770-imageSrc_2a0684a5-ad4d-47fd-acab-45bcf007cdad.png"
        />
        <LandingProductFeature
          title="3. Parandame boileri"
          description="Meie eksperdid teostavad töö kiiresti ja professionaalselt."
          imageSrc="/static/images/1762935301324-imageSrc_140ea933-b1b2-45eb-97d6-105c88b0f79f.png"
        />
      </LandingProductSteps>

      {/* TESTIMONIALS (НОВЫЙ, БЕЗ ФОТО) */}
      <LandingTestimonialGrid
        title="Klientide arvustused"
        description="Vaadake, mida meie kliendid meist arvavad"
        testimonialItems={[
          {
            text: "Teenindus oli kiire, professionaalne ja kvaliteetne. Boiler parandati samal päeval.",
            imageSrc: "",
            name: "Kliendi tagasiside",
            featured: true
          },
          {
            text: "Paigaldustööd olid puhtad ja korrektsed. Väga hea kogemus.",
            imageSrc: "",
            name: "Kliendi tagasiside"
          },
          {
            text: "Aus hind ja super teenus. Soovitan kõigile!",
            imageSrc: "",
            name: "Kliendi tagasiside"
          },
          {
            text: "Hädaolukorras tuldi kohale minuti pealt. Väga professionaalne!",
            imageSrc: "",
            name: "Kliendi tagasiside"
          },
          {
            text: "Boileri hooldus tehti põhjalikult. Vesi muutus kohe puhtamaks.",
            imageSrc: "",
            name: "Kliendi tagasiside"
          },
          {
            text: "Super teenus. Suhtlus kiire ja töö kvaliteetne.",
            imageSrc: "",
            name: "Kliendi tagasiside"
          }
        ]}
        withBackground={false}
        variant="primary"
      />

      {/* CTA BOTTOM */}
      <LandingSaleCtaSection
        title="Saage tasuta hinnapakkumine täna!"
        description="Täitke vorm ja võtame teiega peagi ühendust."
        withBackground
        withBackgroundGlow
        variant="primary"
      >
        <Button size="xl" asChild>
          <Link href="tel:+37253684587">Helista kohe</Link>
        </Button>

        <Button
          size="xl"
          variant="outlinePrimary"
          onClick={() => setContactOpen(true)}
        >
          Saada päring
        </Button>
      </LandingSaleCtaSection>

      {/* BAND */}
      <LandingBandSection
        title="Tallinn ja Harjumaa"
        description="Kiire reageerimine kõikjal piirkonnas"
        variant="primary"
        supportingComponent={
          <>
            <Wrench className="w-12 h-12" />
            <Droplets className="w-12 h-12" />
            <Phone className="w-12 h-12" />
          </>
        }
      />

      {/* FAQ GENERAL */}
      <LandingFaqCollapsibleSection
        title="Korduma kippuvad küsimused"
        description="Vastused kõige sagedamini esitatud küsimustele"
        faqItems={[
          {
            question: "Kui kiiresti saate tulla?",
            answer:
              "Võime tulla samal või järgmisel päeval. Hädaolukorras kohe."
          },
          {
            question: "Kas töödele kehtib garantii?",
            answer:
              "Jah — anname kõikidele töödele garantii."
          },
          {
            question: "Kas töötate nädalavahetustel?",
            answer: "Jah, oleme saadaval 7 päeva nädalas."
          },
          {
            question: "Kas aitate valida boileri?",
            answer: "Jah, soovitame teie vajadustele sobiva mudeli."
          }
        ]}
        withBackground
        variant="primary"
      />

      {/* CONTACT MODAL */}
      <ContactFormDialog open={contactOpen} onOpenChange={setContactOpen} />

      <Footer className="mt-8" />
    </>
  );
}
